...
from .models import AdmissionInfo, AdmissionRegistration, AdmissionDocument
from .forms import AdmissionRegistrationForm
...
def submit_registration(request):
    """
    API xử lý đăng ký tuyển sinh qua AJAX với rate limiting và phone validation.
    Limit: 3 submissions per minute per IP (vì đây là form quan trọng).
    """
    ...
    try:
        ...
        logger.error(f"Error in submit_registration: {e}")
        return JsonResponse({
            'success': False,
